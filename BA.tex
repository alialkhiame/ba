...
\subsection{Kontext}
\begin{center} 
 \includegraphics[width=0.8\textwidth]{lwv.png}
...
Diese Bachelorarbeit wird im Rahmen einer praktischen Zusammenarbeit mit dem \textbf{Landeswohlfahrtsverband Hessen} (LWV Hessen) erstellt. Ziel ist es, ein innovatives System zu entwickeln, das Nutzungsdaten anonymisiert erfasst.
...
\subsubsection{Global Heatmap Service}
Dieser Service übernimmt:
\begin{itemize}
    \item Das Aktivieren und Deaktivieren des Heatmap-Modus.
    \item Das Abfragen relevanter Klickdaten vom Backend.
    \item Das Bereitstellen einer Struktur, die UI-Elemente (\lstinline|element_id|) mit ihren Klickhäufigkeiten verknüpft.
    \item Die Weiterleitung von Änderungen (z.\,B.\ neuer Daten oder geänderter Status) an abonnierende Komponenten und Direktiven.
\end{itemize}

\subsubsection{Overlay Directive}
Die Overlay-Direktive wird in das DOM eingefügt und:
\begin{itemize}
    \item Durchsucht die Seite nach Elementen mit spezifischen IDs.
    \item Erzeugt Overlays (z.\,B.\ eingefärbte Markierungen) auf Basis der Daten aus dem Global Heatmap Service.
    \item Entfernt diese Overlays bei Deaktivierung des Heatmap-Modus.
\end{itemize}

\subsubsection{Admin Toggle Control}
Ein einfacher Administrations-Schalter erlaubt es:
\begin{itemize}
    \item Den Heatmap-Modus über einen Aufruf im Global Heatmap Service zu (de)aktivieren.
    \item Allen abhängigen Komponenten und Direktiven den neuen Status mitzuteilen.
\end{itemize}

\subsubsection{Beispiele für Auswertungen}
Aus den gesammelten \emph{Klick-} und \emph{Fehlerdaten} lassen sich unter anderem folgende Auswertungen ableiten:
\begin{itemize}
    \item \textbf{Klickhäufigkeit pro UI-Element} zur Identifikation besonders häufig genutzter Bereiche.
    \item \textbf{Fehlerquote} (Fehler pro Zeit oder pro Klick), um problematische Formulare oder Buttons zu erkennen.
    \item \textbf{Navigationspfade} (Klick-Sequenzen), die eine Übersicht typischer Benutzerwege liefern.
    \item \textbf{Verweildauer pro Seite}, um Arbeitsaufwände und potenzielle UI-Hürden sichtbar zu machen.
\end{itemize}
...
\subsection{Beispielhafter Angular-Service zur Heatmap-Visualisierung (Kurzfassung)}
...
\noindent
\textbf{Anmerkungen zur Performance:}
\begin{itemize}
