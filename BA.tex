...
\begin{document}
 ...
 \HSFTitle
      ...
      {Herr Zinn Christian}    % Name des Zweitgutachters
...
\addtocontents{toc}{\protect\setcounter{tocdepth}{-1}} % Temporarily suppress TOC depth
\subsection{Abstract}
...
Um die Studienziele zu erreichen, wird zudem eine eingehende Recherche durchgeführt, um zu ermitteln, welche Daten für dieses Vorhaben relevant sind. Dabei wird untersucht, wie die einschlägige Literatur und bestehende Arbeiten die Messung und Definition von User Experience in vergleichbaren Produkten – wie beispielsweise eDok – diskutieren. Ein besonderes Augenmerk liegt hierbei auf der Frage, ob sich langfristige, automatisierte Lösungen realisieren lassen, die ohne direkte Interaktionen zwischen Entwickler*innen und Nutzer*innen auskommen, und wie umfassend sich die UX in einem solchen Setting messen lässt.
...
%--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
% Prevent the Inhaltsverzeichnis from appearing in the Inhaltsverzeichnis
...
\listoffigures
\newpage
\addtocontents{toc}{\protect\setcounter{tocdepth}{4 }} % Restore TOC depth (adjust as needed)
...
Dieser Kontext zeigt die Herausforderungen und Lücken in der aktuellen Arbeitsweise auf und verdeutlicht die Notwendigkeit eines systematischen Ansatzes zur Einbindung der Nutzerperspektive in den Entwicklungsprozess. Die vorliegende Arbeit zielt darauf ab, diese Lücke zu schließen, indem sie ein Modell entwickelt, das die \ac{UX} systematisch erfasst und in die Gestaltung der Software integriert.
...
\ac{UX} ist ein Teilbereich der HCI, der in den frühen 1990er Jahren an Bedeutung gewann \cite{glanznig}. 

 UX konzentriert sich auf die Interaktion zwischen Mensch und System und zielt darauf ab, Produkte und Anwendungen so zu gestalten, dass sie nicht nur funktional sind, sondern auch emotional ansprechend und benutzerfreundlich.

es existieren Unterschiede in der Auffassung dessen, was \ac{UX} genau ist und wie man sie bewertet. In der akademischen Forschung werden zwar zahlreiche Modelle und konzeptionelle Frameworks entwickelt, die UX definieren und messen sollen, doch eine einheitliche, allgemein akzeptierte Definition ist bis heute nicht vorhanden \cite{quelle3}. Immerhin herrscht Konsens darüber, dass \ac{UX} mehr als reine Usability oder Nützlichkeit eines Produkts umfasst, denn UX betrifft auch das subjektive Erleben, das vom Kontext, dem inneren Zustand der Nutzenden und ihren Erwartungen an das Produkt beeinflusst wird \cite{quelle4}.
...
In dieser Arbeit wird \ac{UX} als das Zusammenspiel von Interaktionen, Emotionen und Erwartungen definiert, die Nutzer*innen vor, während und nach der Verwendung einer Anwendung erleben. Dabei lassen sich zwei zentrale Dimensionen unterscheiden: Zum einen die Usability-Aspekte, die sich auf Effizienz, Effektivität und Zufriedenheit fokussieren, und zum anderen die hedonischen Aspekte (wie Ästhetik oder visuelle Anmutung), welche die emotionale und ästhetische Qualität der Anwendung in den Vordergrund stellen.
...
Die Nielsen Norman Group beschreibt Usability als ein Qualitätsmerkmal, das bestimmt, wie leicht und angenehm eine Benutzeroberfläche zu verwenden ist. Nach dieser Definition beinhaltet Usability fünf zentrale Aspekte. Erstens wird die \textbf{Erlernbarkeit} durch den Aufwand bestimmt, den Erstnutzende benötigen, um wesentliche Aufgaben erfolgreich auszuführen. Zweitens bezieht sich die \textbf{Effizienz} auf die Geschwindigkeit, mit der erfahrene Nutzende typische Anwendungsabläufe absolvieren. Drittens umfasst die \textbf{Merkfähigkeit} die Fähigkeit der Nutzenden, gelernte Bedienkonzepte nach längeren Pausen ohne erneute Schulung wieder aufzurufen. Viertens wird unter \textbf{Fehlerrate} die Häufigkeit und Schwere von Fehlern verstanden, inklusive der Frage, wie leicht sich Nutzende von diesen Fehlern erholen können. Schließlich beschreibt die \textbf{Zufriedenheit} das subjektive Empfinden von Komfort und Freude an der Nutzung der entsprechenden Benutzungsschnittstelle.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{gap.png}
    \caption{Die Lücke zwischen Theorie und Praxis nach Hassenzahl}
    \label{fig:gap}
\end{figure}
\textbf{Herausforderungen in der Praxis.} In vielen Entwicklungsprozessen überwiegen nach wie vor klassische \ac{UEM} (Usability Evaluation Methods), die stark auf Effizienz, Effektivität und Fehlerraten abzielen. Diese Methoden genügen jedoch oftmals nicht, um umfassende Aspekte der Nutzererfahrung zu erfassen, wie zum Beispiel die emotionalen, ästhetischen und hedonischen Komponenten. Dadurch entsteht eine Lücke zwischen dem, was in der Forschung als „umfassendes UX-Verständnis“ gefordert wird, und dem, was in der industriellen Realität tatsächlich umgesetzt wird \cite{quelle5}.
...
\textbf{Folgen der Kluft.} Weil viele Unternehmen ihre Entwicklungsprozesse nach wie vor an traditioneller Usability ausrichten und emotionale sowie kontextspezifische Faktoren der UX vernachlässigen, besteht das Risiko, wichtige Innovationschancen zu verpassen. Produkte werden zwar nutzbar, liefern aber nicht unbedingt jenes angenehme, durchgängige Erlebnis, das gerade in wettbewerbsintensiven Märkten zum entscheidenden Alleinstellungsmerkmal werden kann \cite{quelle2}.

\textbf{Ansätze zur Schließung der Lücke.} Um die Kluft zwischen Forschung und Praxis zu überbrücken, braucht es praxisnahe Evaluationsmethoden, die über reine Effizienz- und Effektivitätsmessungen hinausgehen. Hierzu werden unterschiedliche Wege vorgeschlagen:

\begin{itemize}
    \item \textbf{Adaption klassischer Usability-Methoden:} Bestehende Methoden wie Usability-Tests oder Heuristische Evaluation können um UX-relevante Faktoren erweitert werden (z.\,B. durch das gezielte Einbeziehen emotionaler Reaktionen) \cite{quelle5}.
    \item \textbf{Workshops und Austauschformate:} Formate wie der \textit{UXEM}-Workshop im Rahmen der CHI 2008 \cite{quelle5} haben gezeigt, dass ein intensiver Austausch zwischen Forschenden und Industriepraktikern notwendig ist, um Begriffe zu klären und anwendungsnahe UX-Evaluationsmethoden zu entwickeln.
    \item \textbf{Langzeitbeobachtungen unter Realbedingungen:} Statt rein laborbasierter Tests werden zunehmend Studien gefordert, die das Nutzerverhalten über längere Zeiträume hinweg in der echten Nutzungssituation abbilden, um das dynamische Wesen der UX zu erfassen \cite{quelle2, quelle3}.
\end{itemize}

Zusammenfassend zeigt sich, dass UX zwar längst auch in der Industrie als wichtiger Erfolgsfaktor anerkannt ist, aber die praktischen Evaluationsmethoden noch nicht systematisch genug an den umfassenden Erkenntnissen der Forschung ausgerichtet werden. Eine stärkere Verzahnung beider Bereiche und die Entwicklung gemeinsamer Methoden, die sowohl subjektive als auch objektive Aspekte der UX abbilden, sind daher essenziell, um Produkte zu schaffen, welche die Erwartungen und Bedürfnisse der Nutzer*innen wirklich erfüllen \cite{quelle4, quelle5}.
...
\subsubsection{Konsequenzen einer eingeschränkten Beobachtung der \ac{UX} in eDok} \label{subsubsec:konsequenzen_eingeschraenkte_ux}
In dieser Arbeit wurde der Fokus bewusst auf die automatisierte Erfassung und Analyse von Nutzungsdaten gelegt, um vorwiegend funktionale Aspekte (Usability und Effizienz) bewerten zu können. Dieser Ansatz berücksichtigt jedoch hedoniche Aspekte, wie \emph{Ästhetik}, \emph{Freude} oder \emph{emotionale Bindung} zum Produkt, nur unzureichend. Folglich besteht die Gefahr, dass jene Komponenten, die nach Hassenzahl \cite{Hassenzahl2010} den \enquote{SELF}-Charakter eines Produktes ausmachen, vernachlässigt werden.
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.85\textwidth]{arc.png}
  \caption{Hassenzahl modell}
  \label{fig:erd-logging}
\end{figure}
Hassenzahl \cite{Hassenzahl2010} unterscheidet Produkte nach ihrer primären Charakterisierung in \emph{ACT-Produkte} und \emph{SELF-Produkte}. Ein ACT-Produkt (\enquote{stark pragmatisch / schwach hedonic}) wird in erster Linie zur Erreichung konkreter Verhaltensziele entwickelt. Diese Ziele können sich jedoch im Laufe der Zeit oder bei Veränderung der Lebensumstände schnell ändern, wodurch die Bindung des Nutzers an das Produkt abnehmen kann. Im Gegensatz dazu zeichnen sich SELF-Produkte (\enquote{schwach pragmatisch / stark hedonic}) durch ihre enge Verknüpfung mit dem Selbstbild des Nutzers aus (durch Ideale, Erinnerungen oder Beziehungen). Die Wertschätzung für SELF-Produkte ist daher meist beständiger, da sich das Selbstbild der Nutzenden weniger häufig ändert als ihre Verhaltensziele.

Durch das in dieser Arbeit gewählte Vorgehen, primär funktionale Aspekte zu betrachten und hedoniche Kriterien nur am Rande zu erfassen, besteht das Risiko, ein Produkt zu entwickeln, das auf lange Sicht weniger attraktiv bleibt. Es kann zwar kurzfristig nützlich und effizient sein, jedoch fehlen potenzielle Anknüpfungspunkte für eine stärkere emotionale Bindung oder für eine tiefergehende Identifikation der Nutzenden mit dem Produkt. Eine rein pragmatische Bewertung liefert somit nur einen Teil der \ac{UX}: Die \enquote{gefühlte} Qualität, die oft über langfristige Produktloyalität und Zufriedenheit entscheidet, wird kaum in Betracht gezogen.

Daraus lässt sich folgern, dass ein rein auf Usability-Daten basierendes Vorgehen zu einem Produkt führen kann, das vornehmlich als ACT-Produkt wahrgenommen wird. Um jedoch das volle Potenzial einer positiven, nachhaltigen \ac{UX} zu entfalten, sollten langfristig auch hedoniche Aspekte einbezogen werden. Nur so kann ein Design entstehen, das nicht allein auf Effizienz abzielt, sondern auch die Bedürfnisse und Werte der Nutzenden berücksichtigt und dadurch eine stabile Wertschätzung hervorruft.
...
Im ersten Teil dieser Arbeit wurde als Hauptziel definiert, die \ac{UX} in der Anwendung \textit{eDok} systematisch zu erfassen, zu analysieren und durch konkrete Maßnahmen zu verbessern. Dabei sollten folgende Aspekte besonders berücksichtigt werden:
